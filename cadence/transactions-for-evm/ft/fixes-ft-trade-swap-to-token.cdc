...
transaction(
    ...
    prepare(signer: AuthAccount) {
        ...
        let acct = agency.verifyAndBorrowEntrustedAccount(
            methodFingerprint: "fixes-ft-trade-swap-to-token(String|UFix64)",
            params: [symbol, amount.toString()],
        ...
        self.provider = acct.borrow<&FlowToken.Vault>(from: /storage/flowTokenVault)
            ?? panic("Could not borrow reference to the owner's Vault!")

        self.recipient = acct.getCapability<&{FungibleToken.Receiver}>(tokenVaultData.receiverPath).borrow()
